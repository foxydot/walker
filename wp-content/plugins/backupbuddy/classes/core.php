...
class pb_backupbuddy_core {
	...
	public function preflight_check() {
		...
		if ( '' != pb_backupbuddy::$options['last_backup_serial'] ) {
			...
			if ( true !== ( $result = $backup_options->is_ok() ) || ( ! isset( $backup_options->options['updated_time'] ) ) ) {
				pb_backupbuddy::status( 'error', 'Error retrieving fileoptions file `' . pb_backupbuddy::$options['log_directory'] . 'fileoptions/' . pb_backupbuddy::$options['last_backup_serial'] . '.txt' . '`. Err 54478236765.' );
			} else {
	...
	public function final_cleanup( $serial ) {
		...
		if ( true !== ( $result = $backup_options->is_ok() ) ) {
			backupbuddy::status( 'error', 'Unable to open fileoptions file.' );
		}
