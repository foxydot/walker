...
class pb_backupbuddy_fileoptions {
	...
	private $_is_ok = false; // true on error; error message otherwise
	...
	public function load( $ignore_lock = false, $create_file = false ) {
		...
		if ( ( false === $ignore_lock ) && ( true === $this->is_locked() ) ) {
			pb_backupbuddy::status( 'error', 'Error #54555. Unable to read fileoptions file `' . $this->_file . '` as it is currently locked.' );
			$this->_is_ok = 'LOCKED';
			return false;
		}
